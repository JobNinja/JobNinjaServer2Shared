...
class CloudWatchClient:
    ...
    def __init__(
        ...
        if CloudWatchClient._client is None:
            CloudWatchClient._client = boto3.client(
                ...
                config=Config(
                    region_name=AWS_CLOUD_WATCH_REGION
                    if not self._debug
                    else DEBUG_AWS_CLOUD_WATCH_REGION
                ),
            )
    ...
    def submit_value(self, metric_name, val, dimensions=None, unit=None, additional_params={}):
        ...
        try:
            ...
            self._logger.info(
                f"CloudWatchClient::submit_value() {'DEBUG_MODE' if self._debug else ''} - OK  {metric_name}: {val}"
    ...
    def set_alarm(
        ...
        if update_if_exists or not alarm_exists:
            ...
            try:
                ...
                request_id = resp.get("ResponseMetadata", {}).get("RequestId")
                self._logger.info(
                    f"CloudWatchClient::set_alarm() {'DEBUG_MODE' if self._debug else ''} - OK  "
    ...
    def delete_alarm(self, alarm_name):
        ...
        try:
            ...
            request_id = resp.get("ResponseMetadata", {}).get("RequestId")
            self._logger.info(
                f"CloudWatchClient::delete_alarm() {'DEBUG_MODE' if self._debug else ''} - OK  "
    ...
    def get_alarms(self, state="ALARM"):
        res = []
        my_conf = dict(StateValue=state, MaxRecords=100)
        if self._namespace:
        ...
        try:
            ...
            request_id = resp.get("ResponseMetadata", {}).get("RequestId")
            self._logger.info(
                f"CloudWatchClient::get_alarms() {'DEBUG_MODE' if self._debug else ''} - OK  "
    ...
    def _alarm_existing(self, full_alarm_name, metric_name, dimensions={}):
        ...
        try:
            ...
            request_id = resp.get("ResponseMetadata", {}).get("RequestId")
            existing = (
                len([a for a in resp.get("MetricAlarms") if a.get("AlarmName") == full_alarm_name])
                > 0
            )
            self._logger.info(
                f"CloudWatchClient::_alarm_existing() {'DEBUG_MODE' if self._debug else ''} - OK  "
